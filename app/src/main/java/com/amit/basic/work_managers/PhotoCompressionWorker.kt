...
2. Example -> WorkManager to compress an image till a size is reached
...
class PhotoCompressionWorker(
    ...
    override suspend fun doWork(): Result {
        return withContext(Dispatchers.IO){
            ...
            val compressionThresholdBytes = params.inputData.getLong(KEY_COMPRESSION_THRESHOLD, defaultValue = 0L)

            val uri = Uri.parse(stringUri)
            val bytes = appContext.contentResolver.openInputStream(uri)?.use {
                it.readBytes()
            } ?: return@withContext Result.failure()
            //Create bitmap from bytes so that it can be compressed
            val bitmap = BitmapFactory.decodeByteArray(bytes,0,bytes.size)

            var outputBytes: ByteArray
            var quality = 100

            // check quality is less than threshold
            do{
